...
public class SubordinatesRequestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        List<LeaveRequest> requests = leaveRequestService.getSubordinatesRequests(user.getUserId());
        
        // Build HTML
        StringBuilder html = new StringBuilder();
        html.append("<html><body>");
        html.append("<h2>Đơn Nghỉ Phép Của Cấp Dưới</h2>");
        html.append("<p>Manager: ").append(user.getFullName()).append("</p>");
        html.append("<hr>");
        
        if (requests.isEmpty()) {
            html.append("<p>Không có đơn nào.</p>");
        } else {
